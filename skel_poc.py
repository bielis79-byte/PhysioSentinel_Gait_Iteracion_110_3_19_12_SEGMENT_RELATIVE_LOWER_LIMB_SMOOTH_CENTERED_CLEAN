...
# V110.3.19.12 · MINIMAL DIRECT PIPELINE
...
def _segment_relative_lower_seed_112(torch,model,pose,rotvec,trans,scale,target_df,reference_target_df,betas,zero_trans,reference_pose,axis_map,calibration=None):
    """V110.3.19.12 · Retargeting segmentario absoluto respecto al frame semilla.
    ...
    """
    out={'strategy':'V110.3.19.12 segment-relative hip + geometric knee','accepted':False,'chains':{}}
    try:
...
def _fit_skel_sequence(torch, model, motion, seed_fit, start_index=0, iterations=16, progress_cb=None, empirical_profile=None, acquisition_mode="monocular_depth_safe"):
    ...
    prev_tdf=None
    reference_tdf=None
    reference_pose_fixed=prev_pose.detach().clone()
    lower_segment_calib=_lower_segment_calibration_112(torch,model,reference_pose_fixed,prev_rot,prev_trans,scale,betas,zero_trans,axis_map)
    for fi in range(int(start_index),len(frames)):
        ...
        if reference_tdf is None: reference_tdf=tdf.copy()
        if len(rows)<8:
        ...
        if fi>int(start_index):
            ...
            # V110.3.19.12: retargeting segmentario ABSOLUTO respecto al frame 1.
            # No acumula deltas ni exige mejorar el RMSE absoluto; evita deriva y bloqueo por morfología.
            _pose_before_lower=pose.detach().clone()
            pose,lower_temporal=_segment_relative_lower_seed_112(
                torch,model,pose,rotvec,trans,scale,tdf,reference_tdf,betas,zero_trans,
                reference_pose_fixed,axis_map,lower_segment_calib)
            _leg_ids=[3,4,5,6,7,10,11,12,13,14]
            if isinstance(lower_temporal,dict):
                lower_temporal['q_before_lower']={_SKEL_Q_NAMES_130[i]:float(_pose_before_lower[0,i].cpu()) for i in _leg_ids}
                lower_temporal['q_after_lower']={_SKEL_Q_NAMES_130[i]:float(pose[0,i].cpu()) for i in _leg_ids}
        with torch.no_grad():
            world,_,_=_direct_world_130(torch,model,pose,betas,zero_trans,rotvec,scale,trans,axis_map=axis_map)
    ...
    smooth_lower=_smooth_lower_q_sequence_112(torch,model,sequence,betas,zero_trans,axis_map,scale)
    result={'version':'110.3.19.12','retargeting_mode':'segment_relative_lower_limb_frame1_anchored_smoothed','scale':float(scale.cpu()),
    ...
    result['lower_limb_smoothing_112']=_json_safe_134(smooth_lower)
    # V110.3.19.12: las auditorías posteriores a 75/75 son informativas y NUNCA
...
# V110.3.19.12 · SKEL LOWER-LIMB MOTOR ISOLATION TEST
...
def _skel_lower_limb_motor_test_111(torch, model, seed_fit, gender='unknown', n_frames=32):
    ...
    for k in range(n):
        ...
        frames.append({'frame':k+1,'status':'ok','pose':pose[0].cpu().numpy().astype(float).tolist(),
                       ...
                       'motor_test':True})
    seq={'version':'110.3.19.12-motor-test','retargeting_mode':'isolated_skel_lower_limb_motor_test',
...
def _plot_skel_mesh_walker(mesh_seq):
    ...
    try:
        ...
        if Jviz is not None and pelvis_idx is not None and len(Jviz)==len(Vviz):
            ...
            # V110.3.19.12 · orientación VISUAL fija desde frame 1:
        ...
        const layout={{title:'V110.3.19.12 · SKEL ANIMATED ANATOMICAL MESH · anatomical-axis centered playback',margin:{{l:0,r:0,t:45,b:0}},scene:{{aspectmode:'data',xaxis:{{range:P.ranges[0]}},yaxis:{{range:P.ranges[1]}},zaxis:{{range:P.ranges[2]}}}},legend:{{orientation:'h'}}}};
        ...
        st.caption("V110.3.19.12 · visor centrado en pelvis y orientado a ejes anatómicos sólo para presentación. El NPZ científico conserva las coordenadas originales. Usa ‘⬇ Vídeo SKEL’ para grabar y descargar la marcha anatómica desde el visor.")
    except Exception as exc:
...
def _lower_limb_vector_dls_196(torch,model,pose,rotvec,trans,scale,target_df,rows,betas,zero_trans,prev_pose,axis_map,profile,acquisition_mode):
    """V110.3.19.12 · propagación q(t) de miembros inferiores basada en VECTORES DE SEGMENTO.
    ...
    audit={'accepted':False,'strategy':'V110.3.19.12 relative-segment weighted-XYZ DLS','axis_weights':w.astype(float).tolist(),'chains':{}}
    any_commit=False
...
def _lower_limb_delta_tracking_199_fast(torch,model,pose,rotvec,trans,scale,target_df,prev_target_df,betas,zero_trans,prev_pose,axis_map,profile,acquisition_mode):
    """V110.3.19.12 · solver inferior quirúrgico y rápido.
    ...
    try:
        ...
        chains=[('R',[3,4,5,6],('RHip','RKnee','RAnkle'),('femur_r','tibia_r','talus_r')),
                ('L',[10,11,12,13],('LHip','LKnee','LAnkle'),('femur_l','tibia_l','talus_l'))]
        audit={'accepted':False,'strategy':'V110.3.19.12 fast frame-delta DLS','axis_weights':w.tolist(),'chains':{}}
        with torch.no_grad():
        ...
        return pose,{'accepted':False,'strategy':'V110.3.19.12 fast frame-delta DLS','error':f'{type(exc).__name__}: {exc}','non_blocking':True}
...
def render_skel_poc_panel(motion, acquisition_mode="monocular_depth_safe", acquisition_label=None):
    frames=list((motion or {}).get("frames") or [])
    # V110.3.19.12 MINIMAL DIRECT: sin persistencia de secuencia/malla/NPZ.
    if not frames:
    ...
    st.caption("V110.3.19.12 · flujo directo mínimo: sin migración de NPZ y sin recuperación/persistencia de secuencias o mallas previas. Cada ejecución parte de los 75 frames V104/V107 activos y genera q(t) → skin_verts(t) → malla en memoria.")
    ...
    st.markdown("**Modelo SKEL privado · V110.3.19.12 · Lower-Limb q(t) Propagation Fix**")
        ...
        with st.spinner("Instanciando SKEL y ejecutando el primer forward CPU (1 frame)…"):
            ...
            try:
                ...
                try:
                    ...
                    export={
                        "version":"110.3.19.12","frame":int(best_i)+1,"gender":gender,"rmse_before":fit["rmse_before"],"rmse_after":fit["rmse_after"],
                    ...
                    st.markdown("### 🧪 V110.3.19.12 · SKEL Lower Limb Motor Test")
                    ...
                    if isinstance(_mmesh,dict) and isinstance(_mmesh.get('vertices'),np.ndarray):
                        ...
                        st.download_button("⬇️ Descargar Motor Test SKEL (.NPZ)",_mesh_sequence_npz_bytes(_mmesh),"V110_3_19_12_SKEL_lower_limb_motor_test.npz","application/octet-stream",use_container_width=True)

                    st.markdown("### V110.3.19.12 · Propagación temporal 1→75")
                        ...
                        try:
                            ...
                            if len(oks_auto)>=max(2,int(0.9*expected)):
                                ...
                                try:
                                    ...
                                    # V110.3.19.12: fin del camino crítico. No se persiste ni recarga nada aquí.
                    ...
                    if isinstance(seq,dict) and seq.get("frames"):
                        # V110.3.19.12 · sólo memoria de sesión. Si q(t) existe y la malla no,
                        ...
                        st.markdown("### V110.3.19.12 · SKEL ANIMATED ANATOMICAL MESH")
                        ...
                        if isinstance(mesh_seq,dict) and isinstance(mesh_seq.get("vertices"),np.ndarray):
                            ...
                            st.markdown('### V110.3.19.12 · Validación sincronizada Side-by-Side')
                        ...
                        if frame_gate_ok:
                            st.info("✅ El frame semilla está validado. Pulsa el botón anterior para resolver frames 2→75; después V110.3.19.12 generará automáticamente skin_verts(t) y la malla corporal animada en la misma ejecución.")
                        else:
